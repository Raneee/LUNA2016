...
'''
class my_dataset_byInfo00(data.Dataset):
    def __init__(self, train_Index):
        #self.CandidateList = IO_T.makePatientDict(train_Index)
        #self.PatientList = IO_T.makeCandidateList(self.CandidateList, isBalanced=True)
        

        self.PatientDict, self.CandidateList = IO_T.aaaaa(train_Index, isBalanced=True)
        self.size = len(self.CandidateList)
    ...
    def __getitem__(self, idx):
        ...
        img_3D, label, P_ID, XYZ = IMG_T.makeCutImage(self.CandidateList[idx], self.PatientDict)
...
class my_dataset_byInfo(data.Dataset):
    def initialize(self, candidateList, patientList):
        ...
        self.PatientList = copy.deepcopy(patientList)
        self.size = len(self.CandidateList)
